...
type VoteSet struct {
	...
	mtx           sync.Mutex
	votesBitArray *bitarray.BitArray
...
func NewVoteSet(chainID string, height int64, round int, type_ SignedMsgType, valSet *ValidatorSet) *VoteSet {
	...
	return &VoteSet{
		...
		type_:         type_,
		valSet:        valSet,
		votesBitArray: bitarray.NewBitArray(valSet.Size()),
...
type blockVotes struct {
	...
	bitArray  *bitarray.BitArray // valIndex -> hasVote?
...
func newBlockVotes(peerMaj23 bool, numValidators int) *blockVotes {
	return &blockVotes{
		peerMaj23: peerMaj23,
		bitArray:  bitarray.NewBitArray(numValidators),
