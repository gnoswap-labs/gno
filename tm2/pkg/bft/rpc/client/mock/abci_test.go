...
func TestABCIMock(t *testing.T) {
	...
	m := mock.ABCIMock{
		...
		BroadcastCommit: mock.Call{
			...
			Response: &ctypes.ResultBroadcastTxCommit{
				CheckTx:   abci.ResponseCheckTx{ResponseBase: abci.ResponseBase{Data: []byte("stand")}},
				DeliverTx: abci.ResponseDeliverTx{ResponseBase: abci.ResponseBase{Data: []byte("deliver")}},
			},
	...
	assert.Nil(bres.CheckTx.Error)
	assert.EqualValues("stand", string(bres.CheckTx.Data))
	assert.EqualValues("deliver", string(bres.DeliverTx.Data))
}
...
func TestABCIRecorder(t *testing.T) {
	...
	m := mock.ABCIMock{
		Info: mock.Call{Response: abci.ResponseInfo{
			...
			ABCIVersion: "v0.0.0test",
			AppVersion:  "v0.0.0test",
		}},
	...
	_, err = r.ABCIQueryWithOptions("path", []byte("data"), client.ABCIQueryOptions{Prove: false})
	...
	require.True(ok)
	assert.Equal("data", string(ir.Response.Data))
	assert.Equal("v0.0.0test", ir.Response.ABCIVersion)
	assert.Equal("v0.0.0test", ir.Response.AppVersion)
	...
	assert.EqualValues("data", string(qa.Data))
...
func TestABCIApp(t *testing.T) {
	...
	info, err := m.ABCIInfo()
	require.Nil(err)
	assert.Equal(`{"size":0}`, string(info.Response.Data))
	...
	// check the key
	_qres, err := m.ABCIQueryWithOptions("/key", []byte(key), client.ABCIQueryOptions{Prove: true})
	qres := _qres.Response
	require.Nil(err)
